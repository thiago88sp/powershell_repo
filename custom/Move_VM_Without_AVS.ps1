...
#Resource group
$resourceGroupInfoJson = $(az group show --name $sourceResourceGroupName --subscription $sourceSubscriptionId -o json);
...
if ((az group exists --name $targetResourceGroupName --subscription $targetSubscriptionId) -eq 'false') {
    az group create --name $targetResourceGroupName --location $currentLocation --subscription $targetSubscriptionId
    Write-Host ("Resource group {0} does not exist in new subscription, new resource group created" -f $targetResourceGroupName) -ForegroundColor Yellow
}
else {
    Write-Host "Resource Group $targetResourceGroupName already exists in new subscription" -ForegroundColor Green;
}

$targetResourceGroupNameID = "/subscriptions/$targetSubscriptionId/resourceGroups/$targetResourceGroupName";
...
az tag create --resource-id $targetResourceGroupNameID --tags $newTags --subscription $targetSubscriptionId;
...
if($osType -eq 'Windows'){
    ...
    $targetVM = Set-AzVMBootDiagnostic -VM $targetVM -Enable;
